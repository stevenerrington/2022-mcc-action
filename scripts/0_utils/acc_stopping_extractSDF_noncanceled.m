function signal_average = acc_stopping_extractSDF_noncanceled(dirs,dataFiles_beh,dataFiles_neural,behavior)
...
parfor dataFileIdx = 1:length(dataFiles_neural)
    ...
    spk_data = [];
    spk_data = spk_alignTrials(behavior(behaviorIdx).trialEventTimes(:,[3,5,6]),...
    ...
    signal_average{dataFileIdx} = neural_ttmSignalAverage(spk_data,...
        behavior(behaviorIdx).ttm.NC,...
        'units',fieldnames(spk_data),...
        'events',{'target','saccade'},...
        'ssd',ssd_in,...
        'weighting',{'on'});
