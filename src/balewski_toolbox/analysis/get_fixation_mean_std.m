function [u_means, u_stds] = get_fixation_mean_std(rec_dir, current_file, brain_area, trialinfo)
...
aligned_ix = contains(file_names,'pics');
...
[~,w_start] = min(abs(ts - -1000));
[~,w_end] = min(abs(ts - -100));
