function [val_match_pvals, val_non_match_pvals, dir_match_pvals, dir_non_match_pvals  LR_match_betas, LR_match_pvals] =...
    regress_corresponding_and_noncorresponding_PPs_with_FRs_v02(units, ts, t_ch_state_times, t_ch_val, t_unch_state_times, t_unch_val, trial_types, chosen_side)
...
chosen_dir_regressor = [ones(size(all_chState_chSide)), all_chState_chSide];

unchosen_match_regressor = [ones(size(all_unchState_unchVal)), all_unchState_unchVal];
unchosen_dir_regressor = [ones(size(all_unchState_unchSide)), all_unchState_unchSide];
...
chosen_nonmatch_dir_regressor = [ones(size(all_chState_unchSide)), all_chState_unchSide];

unchosen_nonmatch_regressor = [ones(size(all_unchState_chVal)), all_unchState_chVal];
unchosen_nonmatch_dir_regressor = [ones(size(all_unchState_chSide)), all_unchState_chSide];


all_nonmatch_regressors = [chosen_nonmatch_regressor; unchosen_nonmatch_regressor];
all_nonmatch_dir_regressors = [chosen_nonmatch_dir_regressor ; unchosen_nonmatch_dir_regressor];
...
for u = 1:numel(all_chState_FRs(1,1,:))
    ...
    for time = 1: numel(all_chState_FRs(1,:,1))
        ...
        [~,~,~,~,match_stats] = regress(reg_y, all_match_regressors);
        [~,~,~,~,nonmatch_stats] = regress(reg_y, all_nonmatch_regressors);
            
        % extract the pvalues
        val_match_pvals(u, time) =  match_stats(3);
        val_non_match_pvals(u, time) =  nonmatch_stats(3);
                
        % now look at direction
        [~,~,~,~,dir_match_stats] = regress(reg_y, all_match_dir_regressors);
        [~,~,~,~,dir_nonmatch_stats] = regress(reg_y, all_nonmatch_dir_regressors);
    ...
    xt = [1:numel(val_match_pvals(1,:))]*mean(diff(ts)) - 500;
    
    left_match_FRs = mean(all_chState_FRs(all_chState_chSide==1,21:24,u),2);
    right_match_FRs = mean(all_chState_FRs(all_chState_chSide==2,21:24,u),2);
    
    % VALUE
    % now get the matching stats for left and right
    [LEFT_match_b,~,~,~,LEFT_match_stats] = regress(left_match_FRs,...
        [ones(size(left_match_FRs)), all_chState_chVal(all_chState_chSide==1)]);
    
    [RIGHT_match_b,~,~,~,RIGHT_match_stats] = regress(right_match_FRs,...
        [ones(size(right_match_FRs)), all_chState_chVal(all_chState_chSide==2)]);
    ...
    % DIRECTION
    % now get the matching stats for left and right
    [~,left_p,~,left_stats] = ttest(left_match_FRs,0);
    [~,right_p,~,right_stats] = ttest(right_match_FRs,0);

    LR_match_betas(u, 3) =  left_stats.tstat;
    LR_match_betas(u, 4) =  right_stats.tstat;
    
    LR_match_pvals(u, 3) =  left_p;
    LR_match_pvals(u, 4) =  right_p;
