...
sdfWindow = [-999:2000];
blWindow = [0:100];
event = 'stopSignal_artifical';
inputSDF = {signal_collapse.(event).C(:,sdfWindow+1000),...
    signal_collapse.(event).GO(:,sdfWindow+1000)};
...
for session_i = 1:size(dajo_datamap_curated,1)
    for unit_i = 1:size(signal_average_spk{session_i}.individual.ssrt,1)
        count = count+1;
        sdf_ssd_ns_1(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 1}(1,:);
        sdf_ssd_ns_2(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 1}(2,:);
        sdf_ssd_ns_3(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 1}(3,:);
        
        sdf_ssd_c_1(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 2}(1,:);
        sdf_ssd_c_2(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 2}(2,:);
        sdf_ssd_c_3(count,:) = signal_average_spk{session_i}.individual.(event).signal_ssd{unit_i,1}{1, 2}(3,:);
        
    end
end

clear inputSDF_ssd
sdfWindow_plot = sdfWindow;
inputSDF_ssd = {sdf_ssd_c_1(:,sdfWindow_plot+1000),...
    sdf_ssd_c_2(:,sdfWindow_plot+1000),...
    sdf_ssd_c_3(:,sdfWindow_plot+1000),...
    sdf_ssd_ns_1(:,sdfWindow_plot+1000),...
    sdf_ssd_ns_2(:,sdfWindow_plot+1000),...
    sdf_ssd_ns_3(:,sdfWindow_plot+1000)};
sdfTimes_SSD = {sdfWindow_plot,sdfWindow_plot,sdfWindow_plot,sdfWindow_plot,sdfWindow_plot,sdfWindow_plot};
normResp_SSD = scaleResp(inputSDF_ssd,sdfTimes_SSD,'max','-bl',blWindow);

figure('Renderer', 'painters', 'Position', [100 100 1200 200]);hold on
for i = 1:nClusters_manual
    subplot(1,nClusters_manual,i); hold on
    
    plot(sdfWindow_plot,nanmean(normResp_SSD{1}(clusterNeurons{i},:),1), 'color', [colors.canceled 0.3]);
    plot(sdfWindow_plot,nanmean(normResp_SSD{2}(clusterNeurons{i},:),1), 'color', [colors.canceled 0.6]);
    plot(sdfWindow_plot,nanmean(normResp_SSD{3}(clusterNeurons{i},:),1), 'color', [colors.canceled 0.9]);
    
    plot(sdfWindow_plot,nanmean(normResp_SSD{4}(clusterNeurons{i},:),1), 'color', [colors.nostop 0.3]);
    plot(sdfWindow_plot,nanmean(normResp_SSD{5}(clusterNeurons{i},:),1), 'color', [colors.nostop 0.25]);
    plot(sdfWindow_plot,nanmean(normResp_SSD{6}(clusterNeurons{i},:),1), 'color', [colors.nostop 0.25]);
    vline(0, 'k--'); xlim([-200 600])
...
conflictWindow = [0:250]+abs(sdfWindow_plot(1));
